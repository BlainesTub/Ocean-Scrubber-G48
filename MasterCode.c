...
#pragma config(Motor,  port3,           bucket,        tmotorVex393_MC29, openLoop, driveRight)
...
void RobotDrive()
{
	...
	T_test_state driveState = stopped;     //Set initial state to "stopped"
	while(true)
	{
		monitorInput();
		CollectSensorData();
		...
		*/
		...
		case stopped:                            //Stopped Case
			...
			if(button1_pushed == true){
				driveState = forward;               //Change state to forward when button 1 pushed
				button1_pushed = false;              //debounce button1
			}
			if(button2_pushed == true){
				driveState = turnLeft;             //Change state fo backwards when button 2 pushed
		...
		case forward:
			...
			if(SensorValue[sonar] <= 15  && SensorValue[sonar] != -1){      //When the motor position <= 3000

				if(button2_pushed == true){
					driveState = turnLeft;               //When forwards motion is complete, go backwards
					button2_pushed = false;               //Debounce button 2
					}else{
					driveState = stopped;              //If no other input, set state to stopped
		...
		case turnLeft:
			...
			if(getMotorEncoder(imeRight) >= TurnArc){     //Spin until motor position is -3000
				if(button1_pushed == true){
					driveState = forward;                //When backwards motion complete, go forwards
					button1_pushed = false;               //Debounce button 1
					}else{
					driveState = stopped;                //If no other input, set state to stop
				}
				resetMotorEncoder(imeRight);
			}
			button2_pushed = false;                   //Debounce button2
